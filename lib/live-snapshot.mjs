import { CLOCK_SKEW_MS, utcTime, positiveLimit, verifyPayloadHash, validateSourceCounts, snapshotIdentity, uncachedUrl } from './data-integrity.mjs';
...
function validDate(value) {
  try { return value instanceof Date && Number.isFinite(value.getTime()) ? new Date(value.getTime()) : new Date(utcTime(value)); }
  catch { return null; }
}
...
export function validateLiveSnapshot(snapshot, options = {}) {
  ...
  const minimumSuccessRatio = positiveLimit(options.minimumSuccessRatio, 0.2, 'snapshot_coverage_config_invalid', 1);
  const successRatio = validateSourceCounts(snapshot.meta, snapshot.events.length);
  ...
  verifyPayloadHash(snapshot, 'snapshot');
...
export function filterLiveSnapshot(snapshot, window, options = {}) {
  ...
  const now = options.now === undefined ? new Date() : validDate(options.now);
  if (!now) throw new SnapshotError('snapshot_clock_invalid', 'The current time is invalid.');
  const staleAfterMinutes = positiveLimit(options.staleAfterMinutes, DEFAULT_SNAPSHOT_STALE_MINUTES, 'snapshot_freshness_config_invalid');
  ...
  // Scheduled evidence cannot cover the future tail of a rolling 'to now'
...
export async function loadLiveSnapshot({
  ...
  try {
    const response = await fetchImpl(uncachedUrl(url, now), {
      cache: 'no-store',
