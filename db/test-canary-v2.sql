...
create temporary table canary_payload(payload jsonb not null);
insert into canary_payload(payload) values ($payload$
{
  ...
  "events": [
    {
      ...
      "evidencePolicy": { "unsupportedMaterialClaimCount": 0, "mustKnowEligible": true, "disputed": false },
      "evidenceChain": {
...
select public.pulse_persist_scan(payload) from canary_payload;
select public.pulse_persist_scan(payload) from canary_payload;
